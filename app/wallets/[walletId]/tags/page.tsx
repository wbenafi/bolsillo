  ...
  return (
    <main className="page-shell tags-page">
      ...
      <section className="page-heading split">
        ...
        {!wallet.archivedAt && <button type="button" className="button primary" onClick={openCreateDialog}><Plus /> Nuevo tag</button>}
      </section>
      ...
      {tags.length ? (
        ...
        <div className="empty-movements"><h3>Todavía no hay tags</h3><p>Creá etiquetas para clasificar y filtrar los movimientos de este bolsillo.</p>{!wallet.archivedAt && <button type="button" className="button primary" onClick={openCreateDialog}><Plus /> Crear primer tag</button>}</div>
      )}
      <TagDialog
        open={dialogOpen}
        walletId={walletId}
