...
- CRC y USD admiten hasta dos decimales: los totales impresos como `45181.00` y `45181.50` conservan su valor exacto. Los movimientos nuevos usan centésimos enteros con `moneyVersion: 2`; los registros históricos se interpretan según su formato original sin reescribirlos. Ver [compatibilidad y despliegue de montos](docs/money-precision.md). La respuesta de IA se valida por campo: un campo mal formado queda pendiente de revisión y no elimina los otros datos válidos. Una respuesta sin datos utilizables sigue siendo un error.
