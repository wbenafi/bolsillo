...
La dirección implementada, la arquitectura y la evidencia de validación están en [Flujo de movimientos](docs/movement-flow.md). La revisión vigente registra **111 tests en 18 archivos**, lint, typecheck y build aprobados, más **12 escenarios de adjuntos** y 18 capturas móvil/escritorio. El build requirió un reintento por descarga de fuentes. Las pruebas reales usaron `localhost:3034` con servicios locales aislados; la suite estándar main-flow no se ejecutó contra el servidor compartido `localhost:3000` ni se llamó al proveedor externo de IA. El veredicto `ship` resuelve las dos correcciones de recuperación de la revisión inicial y se limita a esa lista, sin una nueva aprobación de toda la superficie.
