...
README.md, app routes, components, Convex domain code, and public brand assets. `docs/ux-review/` preserves the prototype comparison; `docs/movement-flow.md` records the architecture and validation results. Integration with main passed 177 tests across 21 files, lint, typecheck and production build, plus authenticated decimal creation/draft/edit/attachment and statistics drilldown checks on mobile and desktop. Network observation found no mutations before normal edit confirmation and no draft mutations throughout normal edit/cancel/save. The earlier review passed 12 attachment scenarios, including concurrency, permissions, archive state and upload recovery. The external AI provider and standard main-flow Playwright suite against shared `localhost:3000` were not exercised. No production deployment was performed.

## Approved Movement Flow

The user selected B · Paso a paso after comparing HTML prototypes, requested greater prominence for manual entry, and explicitly authorized implementation in the real application with mobile-first UX and maintainable production code. The later request required a rigorous review and no drafts during normal editing. The implemented direction and architecture are recorded in `docs/movement-flow.md`. The review found and resolved two recovery issues involving editor availability and upload retries. `docs/ux-review/` remains the historical comparison; its prototype-only boundary describes that earlier phase and does not limit the later implementation authorization.
